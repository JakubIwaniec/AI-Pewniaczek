...
**Problem:** bez wpisu `event_id` w bundlu meta nie da się zasadnie joinować ani supplementu ani diag — patrz `skipped_no_meta` oraz **`skipped_no_meta_by_dataset`** w diag (`schema_version` **„4’’**).
...
- Diag JSON (schema `"4"`): [`data/integrated/integration_diag_latest.json`](../data/integrated/integration_diag_latest.json) (`baseline_bottleneck_hint`, `skipped_no_meta_by_dataset`, `league_join_scope_table`, `lookup_miss_category_histogram_by_dataset`, …)
